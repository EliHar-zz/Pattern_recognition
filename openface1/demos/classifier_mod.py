...
def infer(args, multiple=False):
	...
	MIN_CONFIDENCE = 70
        if len(reps) > 1:
	    print "I see", len(reps), "faces. Here are the sure", "("+str(MIN_CONFIDENCE)+"%)", "ones from left to right: "
        for r in reps:
            rep = r[1].reshape(1, -1)
            bbx = r[0]
            start = time.time()
            ...
            confidence = predictions[maxI]*100
            if args.verbose:
		...
		if confidence >= MIN_CONFIDENCE:
			if len(reps) == 1:
				print "I see",
			print person, '==>', str(confidence)[:5]+'% confidence.'
		else:
			if len(reps) == 1:
                                print "I'm not sure :(",
	    else:
